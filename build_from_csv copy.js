...
function main(){
  ...
  if (!argv.length){
    console.error('Usage: node build_from_csv.js <file.csv> [--out cgm_data.js]');
    process.exit(1);
  }
  ...
  if (timeIdx === -1 || valueIdx === -1){
    console.error('Could not detect time/value columns. Header:', header);
    process.exit(1);
  }
  ...
  for (const row of rows){
    ...
    const t = parseMaybeDate(row[timeIdx]);
    ...
    const dKey = `${row[timeIdx]}__${mmol.toFixed(2)}`;
