<script>
  ...
  function draw(){
    ...
    // Bottom month labels (adaptive density, single row under the calendar)
    if (showMonthLabels){
  ...
  <div id="controlBar" style="display:flex; align-items:center; gap:12px; flex-wrap:wrap; margin:0 0 6px;">
    ...
    <div style="display:flex; align-items:center; gap:20px; justify-content:flex-end; margin-left:auto;">
      ...
      <div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:flex-end;">
        <button type="button" class={`qbtn ${activeSpan===1 ? 'active' : ''}`} on:click={()=>setSpan(1)}>1 day</button>
        <button type="button" class={`qbtn ${activeSpan===3 ? 'active' : ''}`} on:click={()=>setSpan(3)}>3 days</button>
        <button type="button" class={`qbtn ${activeSpan===7 ? 'active' : ''}`} on:click={()=>setSpan(7)}>7 days</button>
        <button type="button" class={`qbtn ${activeSpan===14 ? 'active' : ''}`} on:click={()=>setSpan(14)}>14 days</button>
        <button type="button" class={`qbtn ${activeSpan===30 ? 'active' : ''}`} on:click={()=>setSpan(30)}>30 days</button>
        <button type="button" class={`qbtn ${activeSpan===90 ? 'active' : ''}`} on:click={()=>setSpan(90)}>90 days</button>
      </div>
    </div>
  </div>
  <canvas bind:this={canvas} style="width:100%; display:block; border:0;"></canvas>
</div>
